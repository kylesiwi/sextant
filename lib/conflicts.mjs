...
async function detectStatuslineCollision(home) {
  if (!home) return null;
  const settingsPath = path.join(home, '.claude', 'settings.json');
  const r = await readJson(settingsPath);
  if (!r.ok) return null;
  const sl = r.value && r.value.statusLine;
  if (!sl || typeof sl !== 'object') return null;
  const cmd = typeof sl.command === 'string' ? sl.command : '';
  if (cmd.length === 0) return null;
  if (cmd.includes('sextant')) return null;
  return {
    kind: 'statusline-collision',
    severity: 'medium',
    location: `${settingsPath}#statusLine.command`,
    message: `~/.claude/settings.json has a non-Sextant statusLine: ${cmd}`,
    action_hint:
      "`~/.claude/settings.json` has a non-Sextant `statusLine`. `/sextant:install-statusline` will prompt before overwriting. If you want to keep the existing one, skip the install step — Sextant's `systemMessage` emissions still surface.",
  };
}
 ...
 * openwolf-graphify-bridge) and other generic hook/statusLine collisions.
 *
 * @param {object} opts
...
export async function detectConflicts({ cwd, home = process.env.HOME ?? '' } = {}) {
  ...
  const [
    ...
    detectStatuslineCollision(resolvedHome),
  ...
  if (statuslineCollision) findings.push(statuslineCollision);
