...
import { stripControlBytes } from '../sanitize.mjs';
...
export function formatMessage(text, { category = 'info', bare = false } = {}) {
  ...
  // `text` can carry repo-sourced content (e.g. a bug's error_message from
  // .sextant/bugs.json), untrusted in a cloned repo. Collapse whitespace FIRST so
  // newlines/tabs become a single separating space, THEN strip control bytes —
  // `\s` covers whitespace controls but NOT ESC/BEL/other C0-C1, which is exactly
  // what could inject terminal escapes. Strip happens before our own styling is
  // applied below. This is the single chokepoint every systemMessage flows through.
  let t = stripControlBytes(String(text).replace(/\s+/g, ' ')).trim();
  if (!t) return '';
