...
//     failure surfaces as `permissionDecision: "ask"` with itemized reasons
//     so Claude can fix-and-retry. Statusline `rules.deny_red` is bumped to
//     signal the deny visually.
...
// kwTermsOf is reused by the [!] write-gate (T4) to extract a rule's kw terms.
import { resolveKeywordMatches, kwTermsOf } from '../retrieval/keywordRules.mjs';
  ...
  if (tool === 'Edit' || tool === 'Write' || tool === 'MultiEdit') {
    ...
    if (cerebrumKind) {
      try {
        ...
        if (gateResult) return gateResult; // ask / deny envelope
      } catch (err) {
        process.stderr.write(`sextant: preToolUse cerebrum-gate err=${err.message}\n`);
      }
    } else {
      // cerebrum-v2 T4 § the [!] write-gate (replaces the deleted [match:]
      // enforcement): a [kw:…][!] rule whose keywords word-boundary-match the
      // changed hunk escalates to permissionDecision:'ask'. Best-effort: any
      // failure degrades to no gating — we never crash the hook.
      try {
        const today = ctx.nowIso().slice(0, 10);
        const gate = await enforceWriteGate({
          cwd: cwdForEdit, payload, tool, sid, today, ctx,
        });
        if (gate) return gate; // ask envelope
      } catch (err) {
        process.stderr.write(`sextant: preToolUse write-gate err=${err.message}\n`);
      }
    }
    // Fall through. Edit/Write don't have any other PreToolUse handling
    // (the Read injection logic below short-circuits on tool !== 'Read'),
    // but we don't return early — that keeps the Phase 0 log invariant.
  }
...
// -----------------------------------------------------------------------------
// cerebrum-v2 T4: the [!] write-gate (replaces the deleted [match:] enforcement)
// -----------------------------------------------------------------------------

// enforceWriteGate: when an Edit/Write/MultiEdit targets a non-cerebrum file,
// scan cerebrum.md for [kw:…][!] safety rules (non-provisional) whose keyword
// terms word-boundary-match the CHANGED HUNK (extractProposedContent — the new
// content, NOT the whole file; spec § "query the changed hunk"). The first
// match returns a permissionDecision:"ask" envelope so the agent surfaces the
// rule before committing. Charter anchor #8: this is the capability the old
// [match:] regex field provided, re-keyed onto [!] (which deletes the
// false-negative/ReDoS surface of an author-supplied regex).
//
// Decision policy: "ask", not "deny" — a false-positive deny blocks legitimate
// work; "ask" hands the choice to the user with the rule body in the reason.
// Start scope = the 4 pinned safety rules; every fire is logged to telemetry.
//
// Returns the envelope on match, or null on no-match / empty store.
async function enforceWriteGate({ cwd, payload, tool, sid, today, ctx }) {
  if (!cwd || typeof cwd !== 'string') return null;
  const toolInput = payload.tool_input ?? null;
  if (!toolInput) return null;

  // 1. The changed hunk the agent wants to commit.
  const hunk = extractProposedContent(toolInput, tool);
  if (hunk === null || hunk === '') return null;

  // 2. Resolve cerebrum.md once. Missing/empty store → no gating.
  let parsed;
  try {
    const cerebrumDir = path.dirname(durableFile(cwd, path.join('cerebrum', 'cerebrum.md')));
    const resolved = await readResolvedCerebrum(cerebrumDir);
    parsed = resolved && resolved.parsed;
  } catch {
    return null;
  }
  ...
  // 3. Candidate gate rules: have a [kw:…] bucket AND the [!] floor, excluding
  // [provisional] (normalized to !review — provisional rules never gate).
  // readResolvedCerebrum normalized [global]→!global+!, [provisional]→!review.
  const lower = hunk.toLowerCase();
  for (const rule of lines) {
    if (!rule || rule.kind !== 'rule' || !Array.isArray(rule.buckets)) continue;
    const buckets = rule.buckets;
    if (!buckets.includes('!')) continue;          // [!] importance floor
    if (buckets.includes('!review')) continue;     // provisional never gates
    if (!buckets.some((b) => typeof b === 'string' && b.startsWith('kw:'))) continue;
    const term = kwTermsOf(buckets).find((t) => keywordPresent(t, lower));
    if (!term) continue;

    // 4. Match: log a fire entry + bump the treatment-arm rule_fires ledger.
    // The permissionDecision fires for BOTH arms (the user experiences the
    // policy regardless of A/B); only the instrumentation is arm-gated.
    await logWriteGateFire({ sid, cwd, rule, term, today, ctx });

    const bodyText = typeof rule.body === 'string' ? rule.body : '(no body)';
    return {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        permissionDecision: 'ask',
        permissionDecisionReason:
          `Sextant: a safety rule matched your change (keyword "${term}")\n` +
          `— ${bodyText}\n\n` +
          `Review the edit; approve only if intentional.`,
      },
    };
  }
...
// extractProposedContent: pull the "new content" the agent wants to commit.
//   Edit       — tool_input.new_string
//   Write      — tool_input.content
//   MultiEdit  — concatenation of every edit's new_string (separated by \n
//                so multi-line regexes don't accidentally bridge edits).
// Returns null on unrecognised shape (defensive).
function extractProposedContent(toolInput, tool) {
...
// logWriteGateFire: append a JSONL line to rules-fired.jsonl AND, on the
// treatment arm, bump stats.rule_fires for the rule's lineHash. The JSONL is
// best-effort (per-fire telemetry, like the existing Read-injection logger);
// stats writes are gated on ab_arm === 'B' to protect the A/B promotion
// ladder (§ 10.5 — control sessions must NOT pollute the ledger).
//
// All I/O here is best-effort: a write failure logs to stderr but never
// crashes the hook.
async function logWriteGateFire({ sid, cwd, rule, term, today, ctx }) {
  // 1. Append a JSONL fire entry. Mirrors the Read-injection logger shape so
  // tail-readers (precompact.json builder) can consume both kinds uniformly.
  try {
    await ensureRuntimeDir(sid, cwd);
    const cerebrumPath = durableFile(cwd, path.join('cerebrum', 'cerebrum.md'));
    const buckets = Array.isArray(rule && rule.buckets) ? rule.buckets : [];
    const kwBucket = buckets.find((b) => typeof b === 'string' && b.startsWith('kw:'));
    let bucketLabel;
    if (kwBucket) bucketLabel = `[${kwBucket}][!]`;
    else if (buckets.includes('!global')) bucketLabel = '[!global]';
    else if (buckets.includes('!')) bucketLabel = '[!]';
    else bucketLabel = '';
    const entry = {
      ...
      source_file: path.relative(cwd, cerebrumPath).split(path.sep).join('/'),
      bucket: bucketLabel,
      event: 'write-gate-block',
      term: (typeof term === 'string' ? term : null),
      today,
    };
    await fs.appendFile(rulesFiredPath(sid, cwd), JSON.stringify(entry) + '\n', 'utf8');
  } catch (err) {
    process.stderr.write(`sextant: preToolUse write-gate jsonl err=${err.message}\n`);
  }

  // 2. Bump rules.blocked + flip deny_red so the statusline signals the
  // block visually (same treatment the cerebrum-gate uses).
  try {
    ...
    process.stderr.write(`sextant: preToolUse write-gate state err=${err.message}\n`);
  }

  // 3. Treatment-arm-only ledger write. Bump rule_fires[<lineHash>] for the
  // matched rule. Control-arm sessions skip this per § 10.5 so suppression
  // bias can't corrupt the ladder.
  try {
    let arm = 'A';
    try {
      const state = await readState(sid, cwd);
    ...
    if (arm === 'B') {
      ...
      const stats = await readStats(base);
      stats.ab_arm = 'B';
      const raw = (rule && typeof rule.raw === 'string') ? rule.raw : null;
      if (raw !== null) incrementRuleFire(stats, lineHash(raw));
      await writeStats(base, stats);
    }
  } catch (err) {
    process.stderr.write(`sextant: preToolUse write-gate stats err=${err.message}\n`);
  }
}
...
//   7. If any failure: format the reasons, bump rules.deny_red, return ask.
...
async function enforceTrancheDocProtection(filePath, cwd) {
  ...
  // 3. Checklist gate: ask if scope file edited before checklist complete.
