...
import { durableBase, durableFile, runtimeBase, turnStatePath, lastJsonPath, userBase } from '../paths.mjs';
...
export default async function sessionStart(payload, ctx) {
  ...
  // Seed / refresh the user-scope ledger probe (best-effort; see recordLedgerProbe).
  if (pluginRoot) {
    try {
      await recordLedgerProbe({ pluginRoot, cwd });
    } catch (err) {
      process.stderr.write(`sextant: sessionStart ledger-probe err=${err.message}\n`);
    }
  }
...
// recordLedgerProbe — write a durable marker into the user-scope sextant dir
// (~/.claude/sextant/), the trust-on-first-use ledger seed.
//
// WHY user scope: a project's `.sextant/` is attacker-controlled in a cloned
// repo, so a "have I managed this project here before?" signal can't live there
// — it has to sit somewhere the clone can't forge. `~/.claude/sextant/` is
// sextant-owned, outside CC's version-pathed, GC'd plugin cache, and (verified)
// survives plugin updates. The in-session agent is sandboxed out of ~/.claude/;
// only hooks (separate, unsandboxed subprocesses) can write here — so this is
// the only legitimate writer, and the "no changes to ~/.claude/sextant" rule
// binds hand-edits, not this runtime hook.
//
// Two jobs today: (1) prove a current-version hook can write user scope at all;
// (2) seed the ledger. `created_*` is WRITE-ONCE so survival across an update is
// unambiguous — after updating, `created_by_version` still names the version
// that first wrote the file. Best-effort: every failure is swallowed by the
// caller; a probe write must never disturb a session.
//
// `home` / `now` are injectable for tests; both default to the live values. The
// write target resolves through userBase() (lib/paths.mjs), which honors
// SEXTANT_USER_BASE / SEXTANT_RUNTIME_BASE so tests never touch the real
// ~/.claude — see that function's precedence contract.
export async function recordLedgerProbe({ pluginRoot, cwd, home, now } = {}) {
  if (!pluginRoot) return;
  const dir = userBase({ home });
  if (!dir) return;
  ...
  try {
    ...
    if (typeof pj.version === 'string' && pj.version) version = pj.version;
  } catch {}

  const file = path.join(dir, 'ledger-probe.json');
  ...
  if (!ledger || typeof ledger !== 'object') {
    ledger = { schema: 1, created_by_version: version, created_ts: ts, session_count: 0, projects: {} };
  }
  if (!ledger.projects || typeof ledger.projects !== 'object') ledger.projects = {};

  // created_* stays frozen (write-once); last_seen_* + counter always refresh.
  ledger.last_seen_by_version = version;
  ledger.last_seen_ts = ts;
  ledger.session_count = (Number(ledger.session_count) || 0) + 1;

  if (cwd) {
    const prev = ledger.projects[cwd];
    if (prev && typeof prev === 'object') {
      prev.last_seen_version = version;
      prev.last_seen_ts = ts;
    } else {
      ledger.projects[cwd] = {
        first_seen_version: version, first_seen_ts: ts,
        last_seen_version: version, last_seen_ts: ts,
      };
    }
  }

  // tmp + rename within the same dir → never leave a torn/partial ledger.
