...
export default async function sessionStart(payload, ctx) {
  ...
  // Keep the user's statusline launcher current. The launcher self-resolves the
  // right plugin version per project at render time, so this only installs /
  // refreshes the stable launcher file and drops a fallback pointer to this
  // version's renderer. No version is ever baked into settings.json.
  if (pluginRoot) {
    try {
      await refreshStatuslineLauncher({ pluginRoot });
    } catch (err) {
      process.stderr.write(`sextant: sessionStart launcher-refresh err=${err.message}\n`);
    }
  }
  ...
  // readTrancheDeliverables) so the nudge can't go stale, and the statusline
  // overwrites state.tranche wholesale from tranches.json (liveTranche). Kept as
  // a hot-state snapshot; no consumer reads this cached field anymore.
...
// Install / refresh the statusline launcher at the stable path the user's
// settings.json references (~/.claude/sextant/statusline.mjs), and drop a
// fallback pointer to THIS version's renderer.
//
// The launcher (statusline/launch.mjs) is version-agnostic: at render time it
// reads installed_plugins.json keyed on the session's cwd and execs the correct
// version's statusline/statusline.mjs. So copying it never goes stale the way
// the old version-baked shim did, and it stays correct even when the plugin is
// installed at different versions across projects (user vs project scope).
//
// Idempotent: only writes when content differs. `home` is injectable for tests;
// it defaults to $HOME then os.homedir(). Best-effort — the caller swallows
// errors. (The plugin writing here at runtime is its own job; the "no changes
// to ~/.claude/sextant" rule binds hand-edits, not this hook.)
export async function refreshStatuslineLauncher({ pluginRoot, home } = {}) {
  const baseHome = home || process.env.HOME || os.homedir();
  if (!baseHome || !pluginRoot) return;

  const launcherSrc = path.join(pluginRoot, 'statusline', 'launch.mjs');
  const renderer = path.join(pluginRoot, 'statusline', 'statusline.mjs');
  const destDir = path.join(baseHome, '.claude', 'sextant');
  const destLauncher = path.join(destDir, 'statusline.mjs');
  const destPointer = path.join(destDir, 'statusline-target');

  let src;
  try {
    src = await fs.readFile(launcherSrc, 'utf8');
  } catch {
    return; // launcher missing from this version → nothing to install
  }

  await fs.mkdir(destDir, { recursive: true });

  let curLauncher = null;
  try { curLauncher = await fs.readFile(destLauncher, 'utf8'); } catch {}
  if (curLauncher !== src) await fs.writeFile(destLauncher, src, { mode: 0o755 });

  let curPointer = null;
  try { curPointer = await fs.readFile(destPointer, 'utf8'); } catch {}
  if ((curPointer ?? '').trim() !== renderer) {
    await fs.writeFile(destPointer, renderer + '\n', 'utf8');
  }
}
