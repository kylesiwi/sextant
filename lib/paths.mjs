...
// userBase: the durable, USER-SCOPE sextant dir (~/.claude/sextant) — home of
// cross-project state like the trust-on-first-use ledger. Distinct from both
// durableBase (per-project, inside the repo, attacker-controllable in a clone)
// and runtimeBase (per-session, tmpfs): this is the one place a cloned repo
// can't reach and that survives plugin updates.
//
// Override precedence mirrors runtimeBase's isolation contract, so tests never
// touch the real ~/.claude:
//   1. SEXTANT_USER_BASE     — explicit override (primary test/power-user knob)
//   2. SEXTANT_RUNTIME_BASE  — when set (every isolation-aware test sets it),
//                              the ledger is redirected under it, namespaced in a
//                              `user-scope/` subdir so it can't collide with the
//                              runtime `sextant-<sid>/` dirs. Zero per-test churn.
//   3. <home>/.claude/sextant — production default (home = $HOME then homedir()).
// Returns null only when no home can be resolved and no override is set.
export function userBase({ home } = {}) {
  const explicit = process.env.SEXTANT_USER_BASE;
  if (explicit && explicit.length > 0) return explicit;
  const runtime = process.env.SEXTANT_RUNTIME_BASE;
  if (runtime && runtime.length > 0) return path.join(runtime, 'user-scope');
  const baseHome = home || process.env.HOME || os.homedir();
  if (!baseHome) return null;
  return path.join(baseHome, '.claude', 'sextant');
}
