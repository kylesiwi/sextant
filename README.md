...
**3. The agent makes an edit (PreToolUse on Edit/Write).** It goes to add the cookie logic in `lib/session.ts`. Because the change contains a guarded keyword (`cookie` / `Set-Cookie`), Sextant injects the matching `[!]` safety rule into the edit's context — so the constraint is in front of the agent exactly when it's editing, and it can't claim it didn't know about the shared session helper. (Earlier versions paused the edit for your approval here; that gate over-fired on ordinary edits and couldn't explain itself at the permission card, so as of v0.44.0 the rule is surfaced as context, not a prompt.) If `lib/session.ts` belonged to a shipped tranche's locked scope, the edit would still be denied outright until the scope was amended — that's an agent-facing block, not a permission wall. The file's node rule is re-injected on the write, too.
...
| `[!]` (mandatory) | adds an exact-match recall floor to a keyword rule — it's injected 100% of the time (never throttled) whenever the keyword appears | safety-critical rules you can't afford to miss |
